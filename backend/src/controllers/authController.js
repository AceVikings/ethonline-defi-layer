...
export const getBalances = async (req, res) => {
  try {
    ...
    const balancePromises = Object.entries(SUPPORTED_CHAINS).map(async ([chainKey, chainConfig]) => {
      try {
        ...
        return {
          ...
          isTestnet: ['sepolia', 'basesepolia', 'arbitrumsepolia', 'optimismsepolia', 'avalanchefuji', 'polygonmumbai'].includes(chainKey),
        };
      } catch (error) {
        console.error(`Failed to fetch balance for ${chainConfig.name}:`, error.message);
        return {
          chainKey,
          chainId: chainConfig.chainId,
          chainName: chainConfig.name,
